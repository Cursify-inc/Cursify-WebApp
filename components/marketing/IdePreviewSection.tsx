"use client"

import { motion, useReducedMotion } from "framer-motion"
import { Bot, CircleCheck, Terminal } from "lucide-react"
import { Container } from "@/components/ui/Container"
import { SectionHeading } from "@/components/ui/SectionHeading"
import { TinyCard } from "@/components/ui/CardVariants"

const codeLines = [
    "agent.plan('refactor auth flow')",
    "sync.devices.validate(currentDevice)",
    "license.lease({ ttl: '15m' })",
    "tools.install('secure-code-review')",
    "integrations.github.connect()",
]

export function IdePreviewSection() {
    const reducedMotion = useReducedMotion()

    return (
        <section className="py-24">
            <Container>
                <SectionHeading
                    eyebrow="Desktop IDE"
                    title="A professional desktop workspace, powered by your web account."
                    description="The Cursify desktop IDE pairs with your account, validates subscription state server-side, and syncs your agent configuration, tools, extensions, settings, and linked account availability."
                />

                <motion.div>
                    <div className="mx-auto max-w-6xl rounded-[2rem] bg-bg-2 p-3 shadow-card">
                        <div className="overflow-hidden rounded-[1.55rem] border border-card-border bg-bg-0">
                            <div className="flex items-center justify-between border-b border-card-border bg-bg-1 px-5 py-4">
                                <div className="flex items-center gap-2">
                                    <span className="h-3 w-3 rounded-full bg-error" />
                                    <span className="h-3 w-3 rounded-full bg-warning" />
                                    <span className="h-3 w-3 rounded-full bg-success" />
                                </div>
                                <div className="text-xs font-medium text-text-tertiary">
                                    cursify.workspace
                                </div>
                            </div>

                            <div className="relative isolate grid min-h-[420px] lg:grid-cols-[230px_1fr_310px]">
                                <aside className="relative z-20 border-r border-card-border bg-bg-1 p-5">
                                    <p className="text-xs font-semibold uppercase tracking-[0.2em] text-text-tertiary">
                                        Explorer
                                    </p>
                                    <div className="mt-5 space-y-3 text-sm text-text-secondary">
                                        {["agents", "tools", "extensions", "sync", "security"].map((item) => (
                                            <TinyCard
                                                key={item}
                                                className="relative w-full"
                                                contentClassName="px-3 py-2"
                                            >
                                                /{item}
                                            </TinyCard>
                                        ))}
                                    </div>
                                </aside>

                                <div className="relative z-10 p-6">
                                    <div className="mb-5 flex items-center gap-2 text-text-primary">
                                        <Terminal className="h-4 w-4 text-brand-primary" />
                                        <span className="text-sm font-semibold">
                                            secure-workflow.ts
                                        </span>
                                    </div>

                                    <div className="space-y-4 font-mono text-sm">
                                        {codeLines.map((line, index) => (
                                            <motion.div
                                                key={line}
                                                initial={reducedMotion ? false : { opacity: 0, x: -16 }}
                                                whileInView={reducedMotion ? undefined : { opacity: 1, x: 0 }}
                                                viewport={{ once: true }}
                                                transition={{ delay: index * 0.12 }}
                                                className="pointer-events-none flex gap-4"
                                            >
                                                <span className="w-6 text-right text-text-tertiary">
                                                    {index + 1}
                                                </span>
                                                <span className="text-text-secondary">{line}</span>
                                            </motion.div>
                                        ))}
                                    </div>
                                </div>

                                <aside className="relative z-20 border-l border-card-border bg-bg-1 p-5">
                                    <div className="flex items-center gap-2">
                                        <Bot className="h-5 w-5 text-brand-primary" />
                                        <p className="font-semibold text-text-primary">Cursify Agent</p>
                                    </div>

                                    <div className="mt-5 space-y-3">
                                        <TinyCard
                                            className="relative w-full"
                                            contentClassName="p-4 text-sm leading-6 text-text-secondary"
                                        >
                                            I found 3 auth flow improvements, 1 risky token refresh
                                            edge case, and 2 missing audit log events.
                                        </TinyCard>

                                        {["Subscription active", "Device paired", "License lease valid"].map(
                                            (item) => (
                                                <TinyCard
                                                    key={item}
                                                    className="relative w-full"
                                                    contentClassName="flex items-center gap-2 px-3 py-2 text-sm text-text-secondary"
                                                >
                                                    <CircleCheck className="h-4 w-4 text-success" />
                                                    {item}
                                                </TinyCard>
                                            )
                                        )}
                                    </div>
                                </aside>
                            </div>
                        </div>
                    </div>
                </motion.div>
            </Container>
        </section>
    )
}
